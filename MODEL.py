...
hidden_1_layer = {
    'weights': tf.get_variable('hl1_W', shape=[900, n_nodes_hl1]),
                  'biases': tf.get_variable('hl1_B', shape=[n_nodes_hl1])}

hidden_2_layer = {
    'weights': tf.get_variable('hl2_W', shape=[n_nodes_hl1, n_nodes_hl2]),
                  'biases': tf.get_variable('hl2_B', shape=[n_nodes_hl2])}

hidden_3_layer = {
    'weights': tf.get_variable('hl3_W', shape=[n_nodes_hl2, n_nodes_hl3]),
                  'biases': tf.get_variable('hl3_B', shape=[n_nodes_hl3])}

output_layer = {
    'weights': tf.get_variable('op_W', shape=[n_nodes_hl3, n_classes]),
...
def hardcoded():
    prediction = neural_network_model(
        [tf.cast(batch_maker.get_xy(2182, 1)[0][0], tf.float32)])

    print(
        sess.run(tf.argmax(sess.run(tf.nn.softmax(sess.run(prediction))), 1)))
...
def classify(arg, isImage=False):
    if isImage:
        ...
        img = resize_data.resize_img(
            os.path.join(BASE_DIR + '/testing', img_path))
