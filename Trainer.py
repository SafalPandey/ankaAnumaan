...
def neural_network_model(data):
    hidden_1_layer = {
        'weights': tf.get_variable('hl1_W', shape=[900, n_nodes_hl1], initializer=tf.random_normal_initializer()),
                      'biases': tf.get_variable('hl1_B', shape=[n_nodes_hl1], initializer=tf.random_normal_initializer())}

    hidden_2_layer = {
        'weights': tf.get_variable('hl2_W', shape=[n_nodes_hl1, n_nodes_hl2], initializer=tf.random_normal_initializer()),
                      'biases': tf.get_variable('hl2_B', shape=[n_nodes_hl2], initializer=tf.random_normal_initializer())}

    hidden_3_layer = {
        'weights': tf.get_variable('hl3_W', shape=[n_nodes_hl2, n_nodes_hl3], initializer=tf.random_normal_initializer()),
                      'biases': tf.get_variable('hl3_B', shape=[n_nodes_hl3], initializer=tf.random_normal_initializer())}

    output_layer = {
        'weights': tf.get_variable('op_W', shape=[n_nodes_hl3, n_classes], initializer=tf.random_normal_initializer()),
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(hm_epochs):
            ...
            for i in range(int(len(input_data.filenames) / batch_size) + 1):
                ...
                # print(len(epoch_y))
                _, c = sess.run(
                    [optimizer, cost], feed_dict={x: epoch_x, y: epoch_y})
                epoch_loss += c
            if (epoch + 1) % 500 == 0:
                saver.save(
                    sess, os.path.join(BASE_DIR, 'models/' + str(epoch + 1) + str(n_nodes_hl1) + 'epochs.txt'))
        ...
        saver.save(
            sess, os.path.join(BASE_DIR, 'models/' + str(hm_epochs) + str(n_nodes_hl1) + 'epochs.txt'))
